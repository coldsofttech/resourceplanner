 ...
 * @param {Object}   [cfg.emptyState]               - { message, link: { href, label } }
...
export function initRenderer(cfg = {}) {
    ...
    function renderPagination(p) {
        ...
        ctrlEl.innerHTML   = _paginationHtml(p);
    }
    ...
    function _emptyHtml({ message, link } = {}) {
        const linkHtml = link ? ` <a href="${link.href}">${link.label}</a>` : '';
        return `
            <tr>
                <td colspan="${colspan}" class="text-center py-5 text-secondary">
    ...
    const pages  = new Set(
        [1, p.total_pages, p.current_page - 1, p.current_page, p.current_page + 1]
            .filter(n => n >= 1 && n <= p.total_pages)
    );
    const sorted = [...pages].sort((a, b) => a - b);

    let html = '';
    html += btn('<i class="bi bi-chevron-double-left"></i>',  1,                  !p.has_previous, false, true);
    html += btn('<i class="bi bi-chevron-left"></i>',         p.current_page - 1, !p.has_previous, false, true);
    ...
    html += btn('<i class="bi bi-chevron-right"></i>',        p.current_page + 1, !p.has_next, false, true);
    html += btn('<i class="bi bi-chevron-double-right"></i>', p.total_pages,      !p.has_next, false, true);
