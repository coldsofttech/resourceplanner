...
document.addEventListener('DOMContentLoaded', () => {
    bsModal = new bootstrap.Modal(document.getElementById('footerModal'));
    ...
    document.getElementById('new-footer-btn').addEventListener('click', () => openModal(null));
    document.getElementById('new-footer-btn-empty')?.addEventListener('click', () => openModal(null));
    ...
    document.getElementById('footerModal').addEventListener('hidden.bs.modal', resetModal);

    loadItems();
});

async function loadItems() {
    const loading = document.getElementById('footers-loading');
    const empty   = document.getElementById('footers-empty');
    const grid    = document.getElementById('footers-grid');

    let data;
    try {
        data = await apiFetch(API_URLS.email_template_footers.list.href, { method: 'GET' });
        allItems = Array.isArray(data) ? data : (data.results ?? []);
    } catch {
        loading.classList.add('d-none');
        showFlash('Could not load footers. Please refresh.', 'danger');
        return;
    }
    ...
    <div class="col-md-6 col-lg-4">
        <div class="et-block-card">
            <div class="et-block-card-header">
                <i class="bi bi-layout-text-window-reverse text-secondary" style="font-size:1.2rem;flex-shrink:0"></i>
...
async function saveItem() {
    ...
    try {
        if (editingId) {
            await apiFetch(API_URLS.email_template_footers.update(editingId).href, {
                method: 'PATCH',
                body: JSON.stringify(payload),
            });
        } else {
            await apiFetch(API_URLS.email_template_footers.create.href, {
                method: 'POST',
                body: JSON.stringify(payload),
            });
        }
        bsModal.hide();
        showFlash('Footer saved.', 'success');
        _refreshList();
    } catch (err) {
...
async function deleteItem() {
    if (!editingId) return;
    if (!confirm('Delete this footer? It will be detached from any templates using it.')) return;
    try {
        await apiFetch(API_URLS.email_template_footers.delete(editingId).href, { method: 'DELETE' });
        bsModal.hide();
        showFlash('Footer deleted.', 'success');
        _refreshList();
    } catch {
...
async function _refreshList() {
    document.getElementById('footers-grid').innerHTML = '';
    document.getElementById('footers-grid').classList.add('d-none');
    document.getElementById('footers-empty').classList.add('d-none');
    document.getElementById('footers-loading').classList.remove('d-none');
    await loadItems();
}
