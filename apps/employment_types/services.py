...
class EmploymentTypeService:
    ...
    def get_employment_type(type_id: int):
        ...
        _type = EmploymentType.objects.get(id=type_id)

        from apps.team_members.models import TeamMember
        members = TeamMember.objects.filter(employment_type=_type)
        _type.total_members = members.count()
        _type.active_members = members.filter(is_active=True).count()
        _type.inactive_members = members.filter(is_active=False).count()
    ...
    @staticmethod
    def list_members(_type_id: int, page=1, page_size=20, include_inactive=False):
        """
        List all members associated with specified employment type
        """
        if not _type_id:
            raise ValidationError("Invalid: type_id must be an integer and greater than 0.")

        _type = EmploymentType.objects.get(pk=_type_id)
        if not _type:
            raise ValidationError(f"Employment type '{_type_id}' does not exist.")
        ...
        if include_inactive:
            ...
            qs = TeamMember.objects.filter(
                is_active=True, employment_type=_type
            ).order_by('last_name', 'first_name')
